...
class ArenaScheduler extends Task {
        ...
        switch ($this->plugin->phase) {
            case Arena::PHASE_LOBBY:
                if(count($this->plugin->players) >= 2) {
                    ...
                    $this->plugin->broadcastMessage("§cWaiting for players...", Arena::MSG_TIP);
                    $this->startTime = 10;
                }
                break;
            case Arena::PHASE_GAME:
                $this->plugin->broadcastMessage("§6Time left: " . Time::calculateTime($this->gameTime) . "", Arena::MSG_TIP);
    ...
    public function reloadSign() {
        ...
        $signText[1] = "§e[ §g" . count($this->plugin->players) . " / " . $this->plugin->data["slots"] . " §e]";
